...
export default function WineCard({ wine, isFavorite, onToggleFavorite, onOpen }) {
  ...
  const shortName =
    wine.wine_name.length > 20
      ? wine.wine_name.slice(0, 18) + '…'
      : wine.wine_name
      ...
      >
        ...
        >
          <svg
            ...
            stroke={isFavorite ? '#E74C3C' : 'rgba(245,240,235,0.25)'}
            strokeWidth="1.8"
          >
        ...
        <p className="mt-1.5 text-[10px] text-white/90 font-medium text-center leading-snug px-0.5">
          {shortName}
        </p>
        {wine.vintage && (
          <p className="text-[9px] text-white/40 mt-0.5">{wine.vintage}</p>
        )}
        <span
          className={[
            'mt-1 text-[8px] px-1.5 py-0.5 rounded-full border',
            wine.drink_now
              ? 'bg-green-900/20 text-green-400 border-green-500/25'
